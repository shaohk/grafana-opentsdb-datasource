...
function (angular, _, dateMath) {
  ...
  function OpenTsDatasource(instanceSettings, $q, backendSrv, templateSrv) {
    ...
    this.tsdbAggAPI = instanceSettings.jsonData.tsdbAggAPI || "";
    this.tsdbTimeStep = instanceSettings.jsonData.tsdbTimeStep || "";
    ...
    this.query = function(options) {
      ...
      _.each(options.targets, function(target) {
        ...
        qs.push(convertTargetToQuery(target, options, start, end, this.tsdbTimeStep, this.tsdbVersion));
      }.bind(this));
    ...
    function convertTargetToQuery(target, options, start, end, timeStep, tsdbVersion) {
      ...
      var rollUpType
      if (timeStep.match(/[0-9]+[smhdwM]?/g)) {
        // 计算时间
        
        var stepMap = {}
        var stepPush = [] 
        // 拆分time step 
        var stepList = timeStep.split(",")
        for (var i in stepList) {
          var step = stepList[i]
          if (step.match(/[0-9]+s/)) {
            stepMap[parseFloat(step)] = step;
            stepPush.push(parseFloat(step))
          }else if (step.match(/[0-9]+m/)) {
            stepMap[parseFloat(step)*60] = step;
            stepPush.push(parseFloat(step)*60)
          }else if (step.match(/[0-9]+h/)) {
            stepMap[parseFloat(step)*3600] = step;
            stepPush.push(parseFloat(step)*3600)
          }else if (step.match(/[0-9]+d/)) {
            stepMap[parseFloat(step)*86400] = step;
            stepPush.push(parseFloat(step)*86400)
          }else if (step.match(/[0-9]+w/)) {
            stepMap[parseFloat(step)*604800] = step;
            stepPush.push(parseFloat(step)*604800)
          }else if (step.match(/[0-9]+M/)) {
            stepMap[parseFloat(step)*2592000] = step;
            stepPush.push(parseFloat(step)*2592000)
          }
        }

        stepPush.sort(function(a,b){
          return b - a
        })

        for (var i in stepPush){
          if ((end - start)/target.rollUpSize/1000 > stepPush[i]) {
            console.log(stepMap[stepPush[i]] + "-" + target.downsampleAggregator);
            rollUpType = stepMap[stepPush[i]] + "-" + target.downsampleAggregator;
            break
          }
        }
      }
      ...
      if (!target.disableDownsampling) {
        var interval =  templateSrv.replace(target.downsampleInterval || (end - start)/target.rollUpSize/1000 + "s");
      ...
      if (target.filters && target.filters.length > 0) {
        ...
        if (query.filters){
          for (var filter_key in query.filters) {
            ...
            query.filters[filter_key].tagk = templateSrv.replace(query.filters[filter_key].tagk, options.scopedVars, 'pipe');
          }
        }
        if (rollUpType) {
          query.filters.push({type: "iliteral_or", tagk: "rollup", filter: rollUpType, groupBy: false});
          query.rate = false;
        }
      } else {
        ...
        if (query.tags){
          for (var tag_key in query.tags) {
            var rpl_key = templateSrv.replace(tag_key.replace('@','$'), options.scopedVars, 'pipe');
            query.tags[rpl_key] = templateSrv.replace(query.tags[tag_key], options.scopedVars, 'pipe');

            if (tag_key != rpl_key){
              delete query.tags[tag_key];
            }
          }
        }
        if (rollUpType) {
          query.tags['rollup'] = rollUpType;
          query.rate = false;
        }
      }
